...
interface MessageDao : BaseDao<Message> {
    ...
    @Query(
        """
            ...
            AND m.category IN('SIGNAL_TEXT', 'PLAIN_TEXT', 'ENCRYPTED_TEXT', 'SIGNAL_DATA', 'PLAIN_DATA', 'ENCRYPTED_DATA','SIGNAL_POST', 'PLAIN_POST', 'ENCRYPTED_POST','SIGNAL_CONTACT', 'PLAIN_CONTACT', 'ENCRYPTED_CONTACT') 
            AND m.status != 'FAILED'
    ...
    @Query(
        """$PREFIX_MESSAGE_ITEM 
            AND (m.category = 'SIGNAL_AUDIO' OR m.category = 'PLAIN_AUDIO' OR m.category = 'ENCRYPTED_AUDIO') 
            AND m.created_at >= :createdAt AND m.rowid > (SELECT rowid FROM messages WHERE id = :messageId) LIMIT 1"""
    )
